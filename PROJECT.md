...
The map reads `https://daten-hamburg.de/gdi3d/datasource-data/Gelaende`, whose `layer.json` advertises geographic/TMS `quantized-mesh-1.0` tiles through level 16. `@loaders.gl/terrain` decodes the source triangles in the browser, and terrain sampling uses barycentric interpolation with a nearest-vertex fallback only inside a covering tile. Terrain UVs are normalized from the source's south-up convention to north-up browser imagery, and synthetic textured skirts stay disabled so map labels cannot flip or smear down tile boundaries. The official service, TopPlus WMS, and satellite export endpoints all allow direct browser image/tile requests, so no proxy is required.
