...
Every map representation uses one vertical datum. The renderer reads Hamburg's CORS-enabled `Gelaende` quantized-mesh DGM hybrid service and samples its Cesium geographic/TMS tiles without drawing them as a second map surface. MapLibre remains the single visible TopPlus or satellite basemap. Each selected LoD is independently normalized from its semantic `GroundSurface` to the terrain elevation at its building root, avoiding source-tier Z offsets during LoD changes. Footprints, block extrusions, detail meshes, placement previews, generated buildings, roads, and official trees use the same absolute terrain elevations.
...
The map reads `https://daten-hamburg.de/gdi3d/datasource-data/Gelaende`, whose `layer.json` advertises geographic/TMS `quantized-mesh-1.0` tiles through level 16. `@loaders.gl/terrain` decodes the source triangles in the browser, and terrain sampling uses barycentric interpolation with a nearest-vertex fallback only inside a covering tile. These triangles are elevation data only: the renderer does not request duplicate terrain imagery or submit the terrain mesh to deck.gl. This keeps TopPlus labels crisp, lets satellite take over consistently at every zoom, and removes the per-frame cost of a second high-triangle surface. MapLibre hides the inactive raster completely and uses a neutral background below satellite imagery, so slow or missing satellite tiles cannot reveal TopPlus labels. The official terrain and basemap services allow direct browser requests, so no proxy is required.
