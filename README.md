...
The built-in Hamburg city-center demo opens automatically with **1,353 buildings**, **1,608 roads**, and **1,042 junctions**. Its editable CityJSON includes the wider LoD2 context and a 68-building LoD3 subset. At the closest zoom the map streams Hamburg's official CORS-enabled, 20 cm textured 3D Tiles directly, while road and junction polygons stay editable CityJSON generated by osm2streets. Close views also show Hamburg's official 3D street trees.
...
The map uses three clear, non-overlapping stages: LoD0 footprints for the overview, grounded source LoD2 in the middle range, and Hamburg's live textured 3D Tiles only when you are very close. Every live tile is attached to the editor's flat map using each building's surveyed ground height, so buildings no longer float while their roof and facade detail remains unchanged. Windows are visible in the official facade photographs even when the editable CityJSON does not encode separate semantic Window or Door surfaces. The buildings come from Hamburg's [official LoD3.0 dataset](https://suche.transparenz.hamburg.de/dataset/3d-gebaeudemodell-lod3-0-hh-hamburg5), and close views add 2,110 measured trees from its official [3D street-tree register](https://metaver.de/trefferanzeige?docuuid=24513F73-D928-450C-A334-E30037945729), published by Freie und Hansestadt Hamburg, LGV under Datenlizenz Deutschland – Namensnennung 2.0. The optional [TopPlusOpen](https://sg.geodatenzentrum.de/web_public/gdz/dokumentation/deu/TopPlusOpen.pdf) basemap is provided by the Federal Agency for Cartography and Geodesy (BKG).
