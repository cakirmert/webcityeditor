...
describe('Hamburg road catalog preparation', () => {
  it('prints a portable, non-mutating local build plan', () => {
    ...
    try {
      ...
      expect(existsSync(resolve(prototypeRoot, '../PREPARE_HAMBURG_ROADS.cmd'))).toBe(true);
      expect(
        existsSync(resolve(prototypeRoot, 'scripts/prepare-hamburg-roads-on-windows.ps1'))
      ).toBe(true);
      const packageJson = JSON.parse(
        readFileSync(resolve(prototypeRoot, 'package.json'), 'utf8')
      );
      ...
      const startupPlan = JSON.parse(
        execFileSync(
          process.execPath,
          [
            resolve(prototypeRoot, 'scripts/dev.mjs'),
            '--catalog',
            'roads',
            '--output-dir',
            completeDir,
