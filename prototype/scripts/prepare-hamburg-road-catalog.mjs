...
const defaultOutputDir = resolve(prototypeRoot, '../Data/hamburg-roads-osm2streets/cityjsonseq');
...
const batchScript = resolve(prototypeRoot, 'scripts/build-hamburg-osm2streets-roads.mjs');
...
async function ensureNodeDependencies() {
  const marker = resolve(prototypeRoot, 'node_modules/proj4/package.json');
  if (existsSync(marker)) return;
  console.log('Installing prototype Node dependencies with npm ci.');
  const npmExecPath = process.env.npm_execpath;
  if (npmExecPath) run(process.execPath, [npmExecPath, 'ci'], { cwd: prototypeRoot });
  else run(process.platform === 'win32' ? 'npm.cmd' : 'npm', ['ci'], { cwd: prototypeRoot });
}
