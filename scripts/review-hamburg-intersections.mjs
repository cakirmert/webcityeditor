...
try {
  ...
  const cases = [0, 2, 12, 17, 177, 182, 302, 144].map(number => {
    const id = `hh-road-r00-c00-osm2streets-intersection-${number}`;
    const junction = index.junctions.find(junction => junction.roadId === id);
    const draft = readRoadJunction(areas, id), plan = buildRoadJunctionPlan({ ...draft, surfaceMode: 'rebuild' }, areas);
    ...
    return { id, name: [...new Set(draft.roadIds.map(id => index.areasByRoadId.get(id)?.[0]?.attributes.roadName).filter(name => name && !name.startsWith('osm2streets')))].join(' / '),
      ...
      source: features(areas.filter(area => area.roadId === id || draft.roadIds.includes(area.roadId)), id), generated: features(plan.areas, id) };
  });
