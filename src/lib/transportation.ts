...
export type { RoadBandKind, RoadDirection, RoadAllowedTurn, RoadIntersectionTurn, RoadVerticalPlacement, RoadCurveMode, RoadGeometryMode, RoadCurveSettings, RoadConnectionTarget, RoadEndpointConnection, RoadVerticalProfile, RoadBand, RoadSectionDraft, RoadDraft } from './road-types';
import type { RoadBandKind, RoadDirection, RoadAllowedTurn, RoadIntersectionTurn, RoadVerticalPlacement, RoadGeometryMode, RoadCurveSettings, RoadConnectionTarget, RoadEndpointConnection, RoadVerticalProfile, RoadBand, RoadSectionDraft, RoadDraft } from './road-types';
      ...
      for (let surfaceIndex = 0; surfaceIndex < geom.boundaries.length; surfaceIndex++) {
        ...
        areas.push({
          ...
          attributes: {
            ...
            junctionRetainedCycleways: (object.attributes?._junctionRetainedCycleways ?? null) as JsonValue,
...
export function removeRoadBandFromDraft(
  ...
  const sections = draft.sections.map((candidate) =>
    candidate.id === sectionId
      ? {
          ...candidate,
          bands: candidate.bands.filter((_, index) => index !== bandIndex),
        }
      : candidate
  );
  return {
 ...
 * Ambiguous cases where several peers point at one source endpoint are left
 * for the existing guarded disconnect path.
 */
export function planStaleReciprocalRoadPropagation(
  ...
  for (const connection of stale) {
    ...
    const sourcePosition =
      connection.sourceEndpoint === 'start'
    ...
    const peerPointIndex =
      connection.endpoint === 'start' ? 0 : peerSection.centerlineWgs84.length - 1;
