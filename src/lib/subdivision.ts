...
export function canSplitBuilding(doc: CityJsonDocument, id: string): {
  ...
  const attrEaveHeight = Number(obj.attributes?._eaveHeight ?? NaN);
  // Buildings generated by this editor store their exact eave height. Prefer
  // that over geometric inference because LoD 2.2 openings add extra Z levels
  // that are not roof/eave levels and can otherwise confuse floor splitting.
  const eaveHeight =
    Number.isFinite(attrEaveHeight) && attrEaveHeight > 0
      ? attrEaveHeight
      : Math.max(0.1, eaveZ - baseZ);
