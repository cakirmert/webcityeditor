...
- `npm test -- --reporter=dot`: **683 passed, 5 skipped** across 81 passing test files and two skipped files. The skipped checks require optional converter/IFC fixtures or the older val3dity executable at the repository's configured external path.
...
Rule profiles are project design policies with documented sources, not complete municipal approval rules. Source conversion does not automatically resize the city. Tree checks depend on loaded tree positions/trunk data; the width policy does not calculate the remaining unobstructed pedestrian envelope around furniture. Changes are local until saved/exported; the hosted site is updated separately by the project's deployment workflow.
